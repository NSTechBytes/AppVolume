# AppVolume-Template (Single Addon)

Minimal starter template for one standalone Novadesk addon.

## Files
- `AppVolume.sln` - Visual Studio solution
- `AppVolume/AppVolume.vcxproj` - Visual Studio project
- `AppVolume/main.cpp` - addon entry points and exported API
- `AppVolume/AppVolume.rc` / `AppVolume/resource.h` - version metadata resource
...
## How to use
1. Open `AppVolume.sln` in Visual Studio 2019+.
...
5. Rename strings in `AppVolume/main.cpp` and `AppVolume/AppVolume.rc` to match your addon name.
